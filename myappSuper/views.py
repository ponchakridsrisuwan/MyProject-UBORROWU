...
def admin_user_deadline(req, id):
    ...
    for user in users:
        if user.token:
            ...
            msg = ['คุณถูกระงับสิทธิ์เป็นระยะเวลา ', obj.deadline, 'วัน เหตุผล : ',
                   obj.reasonfromstaff, 'วันที่ถูกระงับ : ', datetime_th.strftime("%Y-%m-%d %H:%M")]
...
def admin_user_return(req, id):
    ...
    for user in users:
        if user.token:
            ...
            msg = ['สถานะการใช้งานของคุณกลับสู่สถานะปกติเมื่อ ',
...
def admin_user(req):
    ...
    AllUserStudent = User.objects.filter(
        Q(right="นักศึกษา") | Q(status="ปกติ"))
    AllUserStudent_count = User.objects.filter(
        right="นักศึกษา", status="ปกติ")  # count การแนะนำ
    if 'sort' in req.GET:
...
def admin_staff(req):
    ...
    if req.user.phone is None or req.user.token is None:
        return redirect('/phone_add_number')
    AllUserStaff = User.objects.filter(
        Q(right="เจ้าหน้าที่") | Q(right="ผู้ดูแลระบบ") | Q(status="ปกติ"))
    AllUser_count = User.objects.filter(right="เจ้าหน้าที่", status="ปกติ")
    AllUser_count_admin = User.objects.filter(
        right="ผู้ดูแลระบบ", status="ปกติ")
    if 'sort' in req.GET:
        last_sort = req.GET.get('sort', 'default')
        if req.GET['sort'] == 'latest':
...
def admin_block(req):
    ...
    if 'search_user' in req.GET:
        search_user = req.GET['search_user']
        AllUser = AllUser.filter(Q(first_name__contains=search_user) | Q(
            last_name__contains=search_user))
    page_num = req.GET.get('page', 1)
...
#         return render(request, "pages/person_upload.html", context)
...
def person_upload(request):  
    template = "pages/person_upload.html"
    data = Profile.objects.all()
    prompt = {
